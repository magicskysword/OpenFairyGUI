...
import { AUTHORING_OPERATION_SCHEMA, assertAuthoringOperations, authoringPropertySchema } from '../src/authoring/schema.js';
import { Document } from '../src/index.js';

test('authoring contract validates operation envelopes and finite JSON values', t => {
  const operation = { op: 'update', target: { kind: 'node', packageId: 'p', componentId: 'c', nodeId: 'n0' }, props: { x: 10, alpha: 0.5 } };
  t.notThrows(() => assertAuthoringOperations([operation]));
  for (const value of [{ ...operation, unused: true }, { ...operation, props: { x: Infinity } }, { ...operation, target: { ...operation.target, unused: true } }]) {
...
test('authoring contract requires explicit selector counts and XML payloads', t => {
  ...
  t.notThrows(() => assertAuthoringOperations([{ op: 'update', target: { ...target, expectedMatches: 2 }, props: {} }]));
  t.throws(() => assertAuthoringOperations([{ op: 'xml', action: 'insert', target: { kind: 'component', packageId: 'p', componentId: 'c' } }]), { code: 'INVALID_EDIT' });
  t.notThrows(() => assertAuthoringOperations([{ op: 'xml', action: 'attributes', target: { kind: 'component', packageId: 'p', componentId: 'c' }, attributes: { opaque: true } }]));
});
