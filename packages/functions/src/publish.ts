...
export interface PublishOptions {
	...
	/**
	 * Backward-compatible alias for codeGeneration.
	 * When both are provided, codeGeneration takes precedence.
	 */
	generateCode?: boolean;

	/**
	 * Full publish packs atlases before writing runtime artifacts.
	 * Definitions publish skips atlas packing while still emitting package definitions.
	 * Default: 'full'.
	 */
	mode?: PublishMode;
}
...
export function publish(options: PublishOptions): Transform {
	return createTransform('publish', async (doc: Document): Promise<void> => {
		...
		if ((options.codeGeneration ?? options.generateCode) !== false) {
