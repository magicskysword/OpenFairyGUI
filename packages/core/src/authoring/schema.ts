...
const object = (properties: Record<string, AuthoringJsonSchema>, required: string[]): AuthoringJsonSchema => ({ type: 'object', properties, required, additionalProperties: false });
export const AUTHORING_TARGET_SCHEMA = object({
  kind: { enum: ['project', 'package', 'resource', 'component', 'node', 'controller', 'page', 'gear', 'transition', 'transition-item', 'controller-action'] },
  packageId: text, componentId: text, resourceId: text, nodeId: text, controllerName: text, pageId: text, transitionName: text,
  index, selector: text, expectedMatches: index
}, ['kind']);
const scope: AuthoringJsonSchema = { oneOf: [
  { const: 'base' }, object({ controller: text, pageId: text }, ['controller', 'pageId']),
  object({ controller: text, allPages: { const: true } }, ['controller', 'allPages'])
] };
const props: AuthoringJsonSchema = { type: 'object', additionalProperties: true, description: 'Native property patch; omitted fields retain values, null clears optional fields, arrays replace.' };
const shared = { target: AUTHORING_TARGET_SCHEMA };
const operation = (op: string, fields: Record<string, AuthoringJsonSchema>, required: string[] = []) => object({ op: { const: op }, ...shared, ...fields }, ['op', 'target', ...required]);
export const AUTHORING_OPERATION_SCHEMA: AuthoringJsonSchema = {
  oneOf: [
    ...
    operation('xml', { action: { enum: ['insert', 'attributes', 'replace'] }, xml: { type: 'string', minLength: 1 },
      attributes: { type: 'object', additionalProperties: { type: ['string', 'number', 'boolean', 'null'] } },
      bindings: { type: 'object', additionalProperties: text }, index }, ['action'])
  ]
};
...
function matches(schema: AuthoringJsonSchema, value: unknown, path: string): string | undefined {
  if (schema.oneOf) {
    ...
    const selected = schema.oneOf.find(candidate => candidate.properties?.op?.const === (value as { op?: string } | null)?.op);
  ...
  if (schema.type) {
    ...
    if (!types.includes(actual) && !(actual === 'number' && types.includes('integer') && Number.isInteger(value))) return path;
  }
  if (typeof value === 'number' && (!Number.isFinite(value) || (schema.minimum !== undefined && value < schema.minimum) || (schema.maximum !== undefined && value > schema.maximum))) return path;
  if (typeof value === 'string' && ((schema.minLength !== undefined && value.length < schema.minLength) || (schema.maxLength !== undefined && value.length > schema.maxLength))) return path;
  ...
  if (Array.isArray(value) && schema.items) for (let i = 0; i < value.length; i++) {
...
function jsonSafe(value: unknown, depth = 0): boolean {
  ...
  return typeof value === 'object' && value !== null && Object.getPrototypeOf(value) === Object.prototype
    && Object.entries(value).every(([key, item]) => !['__proto__', 'constructor', 'prototype'].includes(key) && jsonSafe(item, depth + 1));
}
...
export function assertAuthoringOperations(value: unknown): void {
  if (!Array.isArray(value) || value.length < 1 || value.length > 200) throw new DocumentEditError('INVALID_EDIT', '编辑批次必须包含 1 至 200 项操作', 'operations');
  for (let i = 0; i < value.length; i++) {
    const operation = value[i];
    const path = `operations[${i}]`;
    const error = matches(AUTHORING_OPERATION_SCHEMA, operation, path);
    if (error || !jsonSafe(operation)) throw new DocumentEditError('INVALID_EDIT', '编辑操作不符合字段契约', error ?? path, operation);
    for (const target of [operation.target, operation.destination].filter(Boolean)) {
      if (target.selector !== undefined && target.expectedMatches === undefined) throw new DocumentEditError('INVALID_EDIT', '选择器需要预期匹配数', `${path}.target.expectedMatches`);
    }
    if (operation.op === 'xml' && (operation.action === 'attributes' ? !operation.attributes || operation.xml !== undefined : !operation.xml || operation.attributes !== undefined)) {
      throw new DocumentEditError('INVALID_EDIT', 'XML 操作缺少匹配的内容', `${path}.${operation.action === 'attributes' ? 'attributes' : 'xml'}`);
    }
  }
}
...
export function authoringPropertySchema(owner: Property): AuthoringJsonSchema {
  ...
  for (const [key, value] of Object.entries(readAuthoringProperties(owner))) {
    properties[key] = value === null ? { type: ['null', 'string', 'number', 'boolean', 'object', 'array'] }
    ...
    if (['x', 'y', 'width', 'height', 'pivotX', 'pivotY'].includes(key)) properties[key]!.description = ['pivotX', 'pivotY'].includes(key) ? 'Normalized pivot coordinate.' : 'Logical pixels.';
  }
