...
function fixture() {
	...
	label.addGear(document.createGear('visibility').setGearType(GearType.Display).setController(controller).setPages('0'));
	const transition = document.createTransition('enter');
	transition.addItem(document.createTransitionItem('rotation').setActionType(TransitionActionType.Rotation).setTargetId('n0'));
...
test('reference graph indexes local transition and gear dependencies', t => {
	...
	t.is(graph.find({ kind: 'page', packageId: 'package1', componentId: 'panel', controller: 'state', id: '0' }).length, 1);
});
